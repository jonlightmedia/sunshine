...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf( '<a rel="nofollow" class="comment-reply-login" href="%s">%s<span class="icon icon-long-arrow-down icon-l-sm"></span></a>',
		...
		$link = sprintf( "<a rel='nofollow' class='comment-reply-link link' href='%s' onclick='%s' aria-label='%s'>%s<span class='icon icon-long-arrow-down icon-l-sm'></span></a>",
...
function get_post_reply_link($args = array(), $post = null) {
	...
	if ( get_option('comment_registration') && ! is_user_logged_in() ) {
		...
		$link = sprintf( "<a rel='nofollow' class='comment-reply-link link' href='%s' onclick='%s'>%s<span class='icon icon-long-arrow-down icon-l-sm'></span></a>",
...
function get_cancel_comment_reply_link( $text = '' ) {
	...
	$formatted_link = '<a rel="nofollow" id="cancel-comment-reply-link" href="' . $link . '"' . $style . '>' . $text . '<span class="icon icon-long-arrow-down icon-l-sm"></span></a>';
	/**
					...
					<div class="comment-metadata">
						...
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
							<time datetime="<?php comment_time( 'c' ); ?>">
								<span>#</span><?php printf( _x( '%1$s', '1: date, 2: time' ), get_comment_date(), get_comment_time() ); ?>
							</time>
						</a>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '' .
		            '<input id="author" name="author" type="text" class="form-control" placeholder="Name (required)" value="' . esc_attr( $commenter['comment_author'] ) . '"' . $aria_req . $html_req . ' /></p>',
		'email'  => '<p class="comment-form-email">' .
		            '<input id="email" name="email" class="form-control"  placeholder="Email (required)"' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-describedby="email-notes"' . $aria_req . $html_req  . ' /></p>',
		'url'    => '<p class="comment-form-url">' .
		            '<input id="url" name="url"  class="form-control"' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' placeholder="Website"  value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>',
	);
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><textarea id="comment" class="form-control" name="comment" cols="45" rows="8"  aria-required="true" placeholder="Your comment here. Be cool" required="required"></textarea></p>',
		...
		'submit_button'        => '<input name="%1$s" type="submit" id="%2$s" class="%3$s btn btn-primary-o btn-rounded" value="%4$s" />',
